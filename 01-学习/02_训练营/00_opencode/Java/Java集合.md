...
1. 常用的集合分类以及他们的区别?
    常见的集合分类首先是分两种，一种是这个collection，然后另外一种是呃map呃，呃，collision下面有Set，嗯，有set集合，有queue队列，还有list列表。Map下面有map。呃，集合的话，它是存放着一个独一无二的元素的队列的话，它就是一个控制先后进去顺序的一个序结构，然后list的列表的话，它是一个有序的，可重复的啊这一个。嗯，几了，然后map的话呢，它就是我们熟悉的这个哈希表啊，比较常见的是这个哈希表表现了这一个kv的呃k value的这一个形式。
2. 你最常用的集合实现类有哪些?
