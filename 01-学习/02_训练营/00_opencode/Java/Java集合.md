...
arraylist由于它是嗯底层是这个数组的结构啊，那么它在嗯这一个中途在他在他在这一个如果是中间有元素要频繁插入呃中间的位置跟头部的话，那么是会涉嫌到呃频繁移动数组元素位置的。啊，所以说它的一个插入效率整体就稍差一点。呃，呃，linked list的话，它底层是一个链表的结构，那么在呃插入的这一个情景下嗯，他就是都是一个Oon的复杂度，只要找到了这一个元素的位置进行插入就行了。嗯，所以总的来说list它更呃，因为因为lease他的这个查询支持随机访问，可以很快的定位到元素link的list，查询的时候要呃on的复杂度去找到元素的位置，所以说list适合这一个查询比较多的情况。Link list的话，它更适合数组元素频繁有插入，删除，修改的情况。
