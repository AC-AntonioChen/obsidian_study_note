...
22. 说下 HashSet 的实现原理?
    嗯，哈希set它就是基于这一个哈希mt实现的，呃，只是它的value都是一个啊这为那的一个状态。嗯，那么哈希map的话，它是呃在这DK.8，它是通过嗯链表哦加这一个红黑树来实现的。啊，是是这个数组加红黑树实现的。那么呃在插入元素的时候，首先要计算呃这个key值的哈希码。嗯，哈希哈希值啊，如果这个哈希值已经存在的话，那么就嗯找到对应的计算出对应的在数组里的槽位，然后去这个呃，比较是否已经在草位上？是否已经有值，如果有的话，那就沿着这个元素。呃，往下找这个链表或是红黑素，红黑素看是不是啊有元素有一样的元素存在。
...
ArrayList跟linked list最大区别在于它们的底层结构，ArrayList是底层结构，是一个object的数组，而linked list的底层是一个双向链表。呃，这个区别的话，主要是在呃首先首先在这一个现场安全上，两者都是非现场安全的，呃，其次在这一个插入上呃数组它只有插入尾部的时间复杂度是O1。嗯插入头部或者插入中间，它的时间复杂度都是o on，因为要移动元素。而链表的话，它的插入就不分位置，都是一个啊都是一个on的复杂度。嗯，那么在这一个呃，查询上呃ArrayList它是支持的一个随呃随机访问的，也就是可以直接拿着一个下标去由o1的复杂度找到这个元素.link的list，它是不支持随机访问的，必须移动到目标位置才能够呃查查到对应的元素，那么就是一个on的查找复杂度。
## Queue
