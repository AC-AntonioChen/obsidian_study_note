...
（1）记录锁：仅仅锁住索引记录的一行。而且有**S锁**(共享锁/读锁：如果一个事务持有了某记录的s锁，其他事务也可以读这个记录，但任何人都不能改记录)和**X锁**（ Exclusive排他锁/写锁：如果一个事务持有了某记录的X锁，其他事务不能读不能写该记录）之分，满足读写互斥，写写互斥。
