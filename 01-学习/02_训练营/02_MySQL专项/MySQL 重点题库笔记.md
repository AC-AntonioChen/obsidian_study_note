...
（2）保持第几页的逻辑，但使用覆盖索引+子查询。子查询语句主要查询分页数据对应数据库的唯一id值(SELECT id FROM table LIMIT 10000, 20)，这样的子查询不需要回表，然后主查询再根据子查询返回的id 进行索引查询完整的数据行。
...
JOIN (
    -- 这里是子查询，重点在于只查 id
    SELECT id FROM users ORDER BY age LIMIT 10000, 20
) t2 ON t1.id = t2.id;
```
### 7.4 如果 SQL 和索引都没问题，查询还是很慢怎么办?
（1）分批查询，大查询拆成多个小查询
（2）增加缓存，将热点数据放到redis
（3）分表，如果表数据量很大，到千万级别，就可以考虑分表了，减少每次查询数据的总量来解决数据查询缓慢的问题。
（4）主从复制，针对读多写少情况。
（5）分库，针对写多读少
## 8 高可用
### 8.1 MySQL 主从复制的过程是怎么样?
- 阶段1： 主库修改数据后会写入binlog日志，从库链接到主库后，主库会创建log dump线程，用于发送binlog内容。
- 阶段2：从库创建专门IO线程用于连接主库的log dump线程，来接受主库的binlog日志，再把binlog写到relaylog 中继日志，给主库返回：“复制成功”
- 阶段3：从库创建回放binlog的sql线程，读relaylog中继日志，回放binlog更新数据，最终实现主从一致。


### 8.2 MySQL 提供了几种复制模式？默认的复制模式是什么?
mysql默认使用异步复制。
（1）同步复制：mysql主库提交事务的线程要等待所有从库的复制成功响应，才会返回客户端结果，性能最差但数据安全性高。
（2）异步复制：mysql主库提交事务的线程不会等待binlog同步到各从库，就返回客户端结果，性能最高，但主库宕机，数据会丢失（**一旦主库宕机，已经被客户端认为‘成功’的数据，如果没传到从库，在从库升级为主库后就会彻底丢失。”**）
（3）半同步复制：介于两者之间，事务线程不用等待所有的从库复制成功响应，只要一部分复制成功响应回来就行，比如一主二从的集群，只要数据成功复制到任意一个从库上，主库的事务线程就可以返回给客户端。这种半同步复制的方式，兼顾了异步复制和同步复制的优点,即使出现主库宕机，至少还有一个从库有最新的数据，不存在数据丢失的风险。
### 8.3 什么是分库分表？什么时候需要分表？什么时候需要分库？
把原本存储在单个数据库上的数据拆分到多个数据库，把原来存储在单个数据表上的数据拆分到多张数据表。
- 分表：单表过大，一般是500w行以上
- 分库：单台mysql扛不住高并发流量，需要分库将请求分散到多台mysql实例。
### 8.4 分库分表后，会产生什么问题？怎么解决？
（1）分布式事务问题：
- 实时性要求高，如金融业务，一般使用TCC事务模型解决问题
- 实时性要求低，如大部分互联网业务，一般使用本地消息表打到最终一致性，性能相对好。
（2）全局ID唯一性问题：分库分表后不能使用自增主键，需使用雪花算法等生成唯一主键id
（3）跨库跨表查询问题：分库分表后查询比较复杂，可以用如下方式解决
- 冗余字段避免跨库关联
- 使用分库分表中间件
- 将数据全量存到es，使用es进行查询
（4）跨库跨表排序问题
- 使用分库分表中间件，分别查询各子表的数据，汇总进行排序
- 将数据全量存到es，使用es进行查询
（5）跨库跨表count
- 计数数据单独存一张表
- 将聚合查询的数据同步到es中，交给es查询