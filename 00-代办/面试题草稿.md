...
HashMap和HashTable的区别主要是HashMap在JDK1.8中引入了红黑树，而hashtable只是简单的数组+链表，不过hashmap是线程不安全的但性能较高，而hashtable则是线程安全的，性能较差。
HashMap和HashSet的区别主要是，hashset基于hashmap实现，hashset维护的是彼此不同的元素集合，而hashmap维护的则是键值对
HashMap 和 TreeMap 区 别主要是hashmap底层使用数组+链表/红黑树，而treemap的底层是红黑树，hashmap是无序的，而treemap是有序的