...
Java 注解（Annotation）是一种为代码添加结构化元数据的机制，本质上是通过@interface语法定义的接口，由编译器或运行时框架解析。它解决了两个核心问题：

‌解耦配置与代码‌
取代传统XML配置（如早期Spring的Bean定义），通过声明式注解（如@Component）直接在代码中嵌入配置信息，提升可读性和维护性。

‌标准化扩展能力‌
为框架提供统一的扩展点：

编译时：@Override校验方法重写逻辑，@NonNull触发空指针静态检查（Lombok）。
运行时：Spring通过@Autowired实现依赖注入，JUnit用@Test标记测试用例，Jackson借@JsonIgnore控制序列化行为。
‌技术价值‌：注解通过反射和APT（Annotation Processing Tool）实现了代码与元数据的低耦合交互，推动Java从"配置驱动"向"约定优于配置"演进，显著减少样板代码，提升开发效率。
