...
在JDK1.7及其之前，HashMap的底层实现使用的是数组+链表，当发生哈希冲突时，也就是数组上已经有元素时，新的元素会通过头插法插到链表上，在出现多个线程并发同时操作同一个索引的头插时，可能会导致链表成环。在极端情况下，hashmap可能会退化成一个链表。

在jdk1.8后 hashmap底层实现使用了数组+红黑树/链表。当发生哈希冲突时，新的元素会通过尾插法插入。当链表元素长度大于等于8时会转红黑树，红黑树长度小于等于6时会退化回链表。

hashmap有几个关键参数，初始容量，负载因子，最大容量。
初始容量跟负载因子都可以在构造函数中传参指定，初始容量决定了hashmap的数组长度，当当前容量大于等于最大容量乘以负载因子时会发生扩容，扩容的规则每次扩容2倍长度，这是因为hashmap计算索引的规则
