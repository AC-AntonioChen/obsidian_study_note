...
### 2.2 MySQL 存储引擎架构了解吗?
Mysql的存储引擎层负责数据存储，索引实现，事务，锁，崩溃恢复等底层操作。存储引擎是可插拔的，可以根据业务需要自行选择合适的引擎，比如innodb支持事务，myisam适合读密集型。这种可插拔是通过统一的接口层实现的，也就是handler层API，server层通过api调用存储引擎层功能

### 2.3 MylSAM 和 InnoDB 的区别
1、事务与锁
- InnoDB：支持事务，行级锁，外键约束。通过MVCC（基于undolog版本链和读视图）实现高并发读
- MyISAM：不支持事务，仅表级锁，无外键约束。写操作会阻塞全表读
2、索引与存储
- ‌**InnoDB**‌：聚簇索引（数据与主键索引绑定），辅助索引保存主键值，查询需回表。
- ‌**MyISAM**‌：非聚簇索引（索引与数据分离，索引存储数据地址），主键/非主键索引结构一致。
3、崩溃恢复
- InnoDB：通过redolog（存储引擎层记录的实际物理操作）和binlog（server层记录的逻辑操作）来实现崩溃恢复和2PC（两阶段提交一致）
- MyISAM：没有事务日志，崩溃后需重建索引或者修复表，也容易丢失数据
4、适用场景
- InnoDB：高并发写，事务安全，适合大部分的业务系统，比如订单服务
- MyISAM：读密集型，无事务支持，适合日志系统
## 3 MySQL 事务
### 3.1 何谓事务
