...
InnoDB从Mysql5.5之后就是默认引擎了，支持事务，行级锁，外键。使用B+树聚簇索引（也就是索引和实际数据列关联在一起）管理数据，使用undolog来实现MVCC（多版本并发控制），使用redolog配合server层执行器的binlog来协调两阶段提交一致性（也就是2PC）。

MyISAM不支持事务，仅支持表级锁，不支持外键。适合读密集型场景。使用B+树非聚簇索引来管理数据（也就是索引和数据列是分别存储的，索引仅保存数据列的地址）。
