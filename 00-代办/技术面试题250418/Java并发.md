...
（3）blocked是阻塞态，当线程因竞争同步锁失败被阻塞，线程会暂停执行，jvm将线程挂起，线程进入操作系统的阻塞队列，所竞争的锁释放后，jvm会自动唤醒该线程去继续竞争锁
（4）waiting是等待态，当线程主动的Obejct.wait()或者Thread.join()进入此状态，需要其他线程去主动唤醒。
（5）timed_waiting是超时等待态，与waiting
