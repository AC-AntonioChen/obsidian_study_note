...
### 什么是 JMM？为什么需要 JMM？ ⭐⭐⭐⭐⭐
#### 什么是JMM
JMM是java内存模型（java memory model ），是Java定义并发编程相关的一种规范，用于描述多线程下线程如何与内存交互，它规定了以下内容：
...
#### 为什么需要JMM
JMM的核心目的是解决多线程编程中的三个问题，并为开发者提供跨平台的一致性保证：
（1）屏蔽硬件差异：不同CPU架构的内存模型差异较大（如缓存一致性、指令重排序策略）。JMM通过统一的规则，隐藏底层复杂性，使Java程序在不同平台表现一致。
